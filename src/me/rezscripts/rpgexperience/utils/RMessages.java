...
public class RMessages {
    ...
    public static void death(String s) {
        for (Player p : RPGCore.plugin.getServer().getOnlinePlayers()) {
            if (RPGCore.plugin.getPD(p) != null && RPGCore.plugin.getPD(p).getOption(RPGOption.DEATH_ALERTS)) {
                p.sendMessage("");
                p.sendMessage(s);
            }
        }
    ...
    public static Class<?> getNMSClass(String name) {
        String version = RPGCore.plugin.getServer().getClass().getPackage().getName().split("\\.")[3];
        try {
