...
public class RSerializer {
    ...
    public static Location deserializeLocation(String s) {
        Location def = RPGCore.plugin.getServer().getWorlds().get(0).getSpawnLocation();
        if (s.length() == 0)
            return def;
        ...
        try {
            ...
            World w = RPGCore.plugin.getServer().getWorld(data[5]);
            if (w == null) {
                RPGCore.plugin.getServer().getWorlds().get(0).getSpawnLocation();
            }
    ...
    public static String serializeItemStack(ItemStack itemStack) {
        ...
        if (str.contains("@")) {
            try {
                RMessages.announce("Error code 109: Item serialization @. Please report this to rezscripts or Xerty.");
