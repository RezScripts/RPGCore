...
import me.rezscripts.rpgexperience.utils.RSerializer;
...
public class PlayerDataFile {
    ...
    public Integer[] getIntArray(String key) {
        if (!values.containsKey(key))
            return (Integer[]) RSerializer.deserializeArray((defaults.get(key).toString()));
        String s = values.get(key);
        return (Integer[]) RSerializer.deserializeArray(s);
    }
    ...
    public void save(final UUID uuid, boolean saveOnMainThread) {
        ...
        if (RPGCore.OFFLINE) { // always save on main thread with filesystem saves, just cuz i'm too lazy to code it otherwise
            File dir = new File(RPGCore.plugin.getDataFolder().getParentFile(), "offlinesaves");
            if (!dir.exists())
                dir.mkdirs();
        ...
        if (saveOnMainThread) {
            ...
            RScheduler.scheduleAsync(RPGCore.plugin, r);
        }
