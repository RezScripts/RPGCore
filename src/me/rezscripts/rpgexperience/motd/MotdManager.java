...
public class MotdManager extends AbstractManagerCore {
    ...
    private static String TEST_REALM_MOTD = ChatColor.AQUA + "RG Private Test Realm";

    private static String motd = null;
    private static String translatedMotd = null;

    public MotdManager(RPGCore plugin) {
        super(plugin);
    }
    ...
    public static void fetchMotd() {
        RScheduler.scheduleAsync(plugin, new Runnable() {
            public void run() {
                ...
                try {
                    scan = new Scanner(stream = new URL("http://www.rpgexperience.net/motd.txt").openStream());
                    String val = "";
